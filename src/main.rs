...
/// going to use a 'Mutex' for inferior mutability
/// 
struct Database{
    map:Mutex<HashMap<String,String>>
}

/// Possible request our clients can send us
    ...
    loop {
        match listener.accept().await{
            Ok((socket,_)) =>{
                ...
                // here to move the ownership f out db handle into the async closure.
    ...
    fn serialize(&self)->String{
        match *self {
            ...
            Response::Set { 
                ref key, 
                ref value, 
                ref previous } => format!("set {} = `{}`, previous: {:?}",key,value,previous),
